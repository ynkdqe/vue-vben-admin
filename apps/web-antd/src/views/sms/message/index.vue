...
const query = reactive({ keyword: '', status: undefined as number | string | undefined, smsProviderId: undefined as string | number | undefined, smsTemplateId: undefined as string | number | undefined, page: 1, pageSize: 20 });
...
const columns: TableColumnsType = [
  ...
  { title: 'Provider', dataIndex: 'smsProviderId', key: 'smsProviderId', width: 160 },
  { title: 'Template', dataIndex: 'smsTemplateId', key: 'smsTemplateId', width: 160 },
  { title: 'Phone', dataIndex: 'phoneNumber', key: 'phoneNumber', width: 140 },
  { title: 'Message', dataIndex: 'message', key: 'message', ellipsis: true },
  { title: 'Status', dataIndex: 'status', key: 'status', width: 120 },
  { title: 'Thời gian gửi', dataIndex: 'sendedTime', key: 'sendedTime', width: 180 },
  { title: 'Ngày tạo', dataIndex: 'creationTime', key: 'creationTime', width: 180 },
];

async function loadProviders() {
  try {
    const res = await fetchSmsProviderList({ page: 1, pageSize: 1000 });
    providerMap.value = Object.fromEntries(res.items.map(p => [String(p.id), p.name || String(p.id)]));
  } catch (err) {
...
async function loadTemplates() {
  try {
    ...
    templateMap.value = Object.fromEntries(res.items.map(t => [String(t.id), t.name || String(t.id)]));
  } catch (err) {
...
async function loadData() {
  ...
  try {
    ...
    message.error(err?.message || 'Không tải được danh sách message');
  } finally {
    loading.value = false;
  }
}
...
function handleReset() { query.keyword = ''; query.status = undefined; query.smsProviderId = undefined; query.smsTemplateId = undefined; query.page = 1; query.pageSize = 20; loadData(); }

onMounted(() => { loadProviders(); loadTemplates(); loadData(); });
...
<template>
    ...
    <div class="rounded-md p-4 shadow-sm search-card">
      <AForm :layout="isMobile ? 'vertical' : 'inline'" @submit.prevent>
        ...
        <AFormItem label="Provider" :style="isMobile ? { width: '100%' } : {}">
          <ASelect v-model:value="query.smsProviderId" allow-clear placeholder="Tất cả" :style="isMobile ? { width: '100%' } : { width: '220px' }">
            <ASelectOption v-for="(name, id) in providerMap" :key="id" :value="id">{{ name }}</ASelectOption>
          </ASelect>
        </AFormItem>

        <AFormItem label="Template" :style="isMobile ? { width: '100%' } : {}">
          <ASelect v-model:value="query.smsTemplateId" allow-clear placeholder="Tất cả" :style="isMobile ? { width: '100%' } : { width: '220px' }">
            <ASelectOption v-for="(name, id) in templateMap" :key="id" :value="id">{{ name }}</ASelectOption>
          </ASelect>
        </AFormItem>
        ...
        <AFormItem :style="isMobile ? { width: '100%' } : {}">
          <ASpace :wrap="true" :style="isMobile ? { width: '100%' } : {}">
            ...
            <AButton :block="isMobile" type="primary" ghost @click="sendFormVisible = true">Gửi tin nhắn</AButton>
          </ASpace>
        </AFormItem>
      </AForm>
    </div>

    <ATable :columns="columns" :data-source="dataSource" :loading="loading" :pagination="{ current: query.page, pageSize: query.pageSize, total, showSizeChanger: true }" row-key="id" @change="(p) => { query.page = p.current!; query.pageSize = p.pageSize!; }">
  ...
  <SendSmsForm :visible="sendFormVisible" @update:visible="(v)=> sendFormVisible = v" @sent="loadData" />
  </div>
</template>
